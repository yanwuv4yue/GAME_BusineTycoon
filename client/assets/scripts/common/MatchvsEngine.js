...
MatchvsEngine.prototype.init = function(channel, platform){
    ...
    let result = mvs.engine.init(mvs.response, channel, platform,global.gameID, global.appKey, global.gameVersion);
...
MatchvsEngine.prototype.joinRandomRoom = function(mxaNumer,profile){
    let result = mvs.engine.joinRandomRoom(mxaNumer, MatchvsEngine.prototype.getUserProfile(profile));
...
MatchvsEngine.prototype.leaveRoom = function () {
    let obj = {name: global.name, profile: '主动离开了房间'};
    let result = mvs.engine.leaveRoom(JSON.stringify(obj));
    console.log(global.name+"主动离开房间result"+result);
    return result;
};
...
MatchvsEngine.prototype.joinRoom = function (roomID,profile) {
    let result = mvs.engine.joinRoom(roomID, MatchvsEngine.prototype.getUserProfile(profile));
...
MatchvsEngine.prototype.getUserProfile = function (profile) {
    let userProfile = {name: global.name, avatar: global.avatar, profile: profile};
